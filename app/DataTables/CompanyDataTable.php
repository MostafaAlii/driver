...
class CompanyDataTable extends BaseDataTable {
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable {
        return (new EloquentDataTable($query))
            ...
            ->rawColumns(['action','created_at', 'updated_at', 'name','status']);
    ...
    public function query(): QueryBuilder{
        if(get_user_data()->type == 'general')
            return Company::with(['profile','country'])->orderBy('id', 'ASC');
        return Company::with(['profile','country'])->whereCountryId(get_user_data()->country_id)->orderBy('id', 'ASC');
    }
